...
class Decoder_Handler(Basement_Handler):
    ...
    def initial_parameter(self):
        ...
        self.W = int(config.get('num_width', 256))
        self.H = int(config.get('num_height', 256))
        self.nC = int(config.get('num_channel', 28))
    ...
    def data_assignment(self,dataset_dir,is_training=True):
        ...
        #self.train_index, self.valid_index, self.test_index, self.sense_mask = Data_Division(dataset_name)
    ...
    def train(self):
        ...
        while epoch_cnt <= self.epochs:
            ...
            for trained_batch in range(0,self.train_size):
                ...
                if trained_batch%2 == 0 and trained_batch != 0:
    ...
    def test(self):
        ...
        recon = np.zeros((self.test_size*self.batch_size, self.H, self.W, self.nC))
        truth = np.zeros((self.test_size*self.batch_size, self.H, self.W, self.nC))
        for tested_batch in range(self.test_size):
            ...
            truth[tested_batch*self.batch_size:(tested_batch+1)*self.batch_size,:,:,:] = ground_test
    ...
    def psnr(self, img, ref):           # img, ref {10,256,256,28}
        ...
        for i in range(ref.shape[0]):
            ...
            for ch in range(ref.shape[3]):
                mse = np.mean((img[i,:,:,ch] - ref[i,:,:,ch])**2)
                total_psnr += 20 * math.log10(PIXEL_MAX / math.sqrt(mse))
            psnr_list.append(total_psnr/ref.shape[3])
        return psnr_list