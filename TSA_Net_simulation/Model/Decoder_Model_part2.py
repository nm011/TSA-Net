...
class Depth_Decoder(Basement_TFModel):
    ...
    def __init__(self, value_sets, init_learning_rate, sess, config, is_training=True, *args, **kwargs):
        ...
        '''
        Arguments:
            measurement: one-frame measurement segment, [batch_size, hcube, wcube, 1]
            mat_sense: sensing matrix, [1, hcube, wcube, depth]
            cube_truth: the decoded cube image / groundtruth, [batch_size, hcube, wcube, depth]            
        '''
    ...
    def encdec_handler(self, measurement, mat_sense):
        ...
        self.hyper_structure = [(3,64,3,2),(3,128,3,2),(3,256,3,2),(3,512,3,2),(3,1024,3,2)]
        self.end_encoder = (3,1280,3)
        
        encoder_in = measurement
        #print(encoder_in.get_shape().as_list())
        output = self.inference(encoder_in,0.8,phase_train = self.is_training)#self.is_training
        return output
    ...
    def metric_opt(self, model_output, ground_truth):
        ...
        elif self.loss_func == 'CASL':
            self.loss = (loss_content_aware_spectral(model_output, ground_truth) / 100) + loss_multi_scale_structure(model_output, ground_truth) + loss_rmse(model_output, ground_truth) + loss_spec(model_output, ground_truth)
        elif self.loss_func == 'MSSL':
            self.loss = loss_multi_scale_structure(model_output, ground_truth)
        else:
    ...
    def Transform_layer(self, att_input, module_idx, dim_lambda, att_unit, num_heads, mode=[1,1], mask=None, flat_attention=False):
        ...
        # print('net', net.get_shape().as_list()) #[5, 256, 256, 64] Verified
